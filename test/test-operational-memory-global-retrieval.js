...
try {
  ...
  const inherited = lesson(firstB, 'fetch_required_git_refs');
  assert.ok(inherited, 'unrelated project should inherit a whitelisted semantic lesson');
  assert.equal(inherited.scope, 'global');
  assert.ok(inherited.relevanceReason.startsWith('global_'));
  ...
  console.log('✅ Operational Memory M3B safe global retrieval tests passed');
} finally {
