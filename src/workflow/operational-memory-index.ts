...
function readIndexState(db: SqliteDatabase): IndexState | null {
  const row = db.prepare(
    'SELECT schema_version, indexed_through_offset, authority_size_bytes, authority_mtime_ms, record_count FROM index_state WHERE id = 1',
  ).get();
  if (!row) return null;
  return {
...
function validateSchema(db: SqliteDatabase): void {
  const requiredEventColumns = new Set([
    'record_sequence', 'start_offset', 'end_offset', 'event_id', 'workflow_id',
  ...
  for (const column of [
    ...
    'authority_mtime_ms', 'record_count', 'rebuild_status',
  ]) {
...
async function* readJournalRecords(
  ...
  try {
    ...
    if (pending.length > 0) {
      ...
      if (line.length > 0) {
        yield { line: line.toString('utf8'), startOffset: pendingStart, endOffset };
      }
    }
  } finally {
    await handle.close();
...
function insertEvent(
  ...
  db.prepare(`
    INSERT INTO events (
      record_sequence, start_offset, end_offset, event_id, workflow_id, kind,
      reason_code, lesson_code, source_tool, family, stage_id, fingerprint, occurred_at
    ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
  `).run(
    recordSequence, record.startOffset, record.endOffset, event.id, event.workflowId,
...
function updateGroup(
  ...
  db.prepare(`
    INSERT INTO groups (
      ...
      latest_event_id, latest_record_sequence
    ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, 1, ?, ?)
  ...
  `).run(
    ...
    event.occurredAt, event.occurredAt, event.id, recordSequence,
  );
}
...
async function journalStat(memoryPath: string): Promise<{ size: number; mtimeMs: number }> {
  const stat = await fs.stat(memoryPath).catch((error) => {
    if ((error as NodeJS.ErrnoException).code === 'ENOENT') return null;
    throw error;
  });
  return stat ? { size: stat.size, mtimeMs: stat.mtimeMs } : { size: 0, mtimeMs: 0 };
}
async function ingestRange(
  ...
  try {
    ...
    writeIndexState(db, endOffset, endOffset, authorityMtimeMs, recordCount);
    db.exec('COMMIT');
    return recordCount;
  } catch (error) {
...
async function rebuildIndex(
  ...
  try {
    ...
    if (stat.size > 0) {
      await ingestRange(db, memoryPath, 0, stat.size, 0, parseLine, stat.mtimeMs);
    } else {
      writeIndexState(db, 0, 0, 0, 0);
    }
    validateSchema(db);
    buildComplete = true;
  } finally {
...
async function synchronizeExistingIndex(
  ...
  try {
    ...
    if (
      stat.size === state.indexedThroughOffset &&
      stat.size === state.authoritySizeBytes &&
      stat.mtimeMs !== state.authorityMtimeMs
    ) return false;
    if (stat.size === state.indexedThroughOffset) return true;
...
export async function updateOperationalMemoryIndexAfterAppend(
  ...
  return synchronizeExistingIndex(memoryPath, indexPath, parseLine);
}
export async function ensureOperationalMemoryIndex(
  ...
  if (existing?.isFile()) {
    const synchronized = await synchronizeExistingIndex(memoryPath, indexPath, parseLine);
    if (synchronized) return true;
  } else if (existing) {
    return false;
  }
  try {
    return await rebuildIndex(memoryPath, indexPath, parseLine);
  } catch {
    return false;
  }
}
...
export async function readOperationalMemoryIndexEvents(
  ...
  if (!await ensureOperationalMemoryIndex(memoryPath, indexPath, parseLine)) return null;
  const db = openDatabase(indexPath);
  if (!db) return null;
  try {
    validateSchema(db);
    ...
    const rows = db.prepare(`
      SELECT record_sequence, start_offset, event_id, workflow_id, kind,
        reason_code, lesson_code, source_tool, family, stage_id, fingerprint, occurred_at
      FROM events
