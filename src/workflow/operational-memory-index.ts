...
function readIndexState(db: SqliteDatabase): IndexState | null {
  const row = db.prepare(
    'SELECT schema_version, indexed_through_offset, authority_size_bytes, authority_mtime_ms, record_count, project_id, repository_id, authority_chain_hash FROM index_state WHERE id = 1',
  ).get();
  if (!row) return null;
  return {
...
function validateSchema(db: SqliteDatabase): void {
  ...
  for (const column of [
    ...
    'authority_mtime_ms', 'record_count', 'project_id', 'repository_id',
...
async function ingestRange(
  ...
  try {
    ...
    writeIndexState(
      db, indexedThroughOffset, endOffset, authorityMtimeMs, recordCount,
  ...
  try {
    ...
    if (stat.size > 0) {
      await ingestRange(
        ...
        parseLine, stat.mtimeMs, scope,
      );
    } else {
      writeIndexState(db, 0, 0, 0, 0, scope, EMPTY_AUTHORITY_CHAIN);
    }
    validateSchema(db);
    buildComplete = true;
  } finally {
...
async function synchronizeExistingIndex(
  ...
  try {
    ...
    const metadataUnchanged =
      stat.size === state.authoritySizeBytes && stat.mtimeMs === state.authorityMtimeMs;
    ...
    if (stat.size === state.indexedThroughOffset) {
      writeIndexState(
        db, state.indexedThroughOffset, stat.size, stat.mtimeMs, state.recordCount,
...
export async function updateOperationalMemoryIndexAfterAppend(
  ...
  return synchronizeExistingIndex(memoryPath, indexPath, parseLine, scope, true);
}
