...
export async function runDesktopCommanderServer() {
  try {
    ...
    server.oninitialized = () => {
      ...
      // Flush all deferred messages from both the runtime and server
...
export function startDesktopCommanderServer(): void {
  void runDesktopCommanderServer().catch(async (error) => {
  const errorMessage = error instanceof Error ? error.message : String(error);
